...
- `make release` needs a GitHub token in the environment, so run it as:
  ...
  The shell does not export `GITHUB_TOKEN` — `gh` keeps its token in the keyring rather
  than the environment — so `$(gh auth token)` supplies it. Without it goreleaser stops
  with `missing GITHUB_TOKEN, GITLAB_TOKEN and GITEA_TOKEN`.

  Nothing else needs unsetting. `GORELEASER` in the Makefile is
  `env -u GITLAB_TOKEN -u GITEA_TOKEN goreleaser`, so both `smoke` and `release` run
  goreleaser without the other forges' tokens and leave the calling shell alone. That is
  there because goreleaser refuses to guess when it can see tokens for more than one
  forge, failing with `multiple tokens found` — which is what stopped 0.12.2 before it
  built anything.
