...
func (c *ProviderClient) Enabled() bool {
	return c.Session.Providers.Linode.Enabled
}
...
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if os.Getenv("CCI_BACKUP_RESPONSES") == "true" {
		c.Logger.Debug("backing up linode host report")

		if err = os.WriteFile(fmt.Sprintf("%s/backups/linode_%s_report.json", session.GetConfigRoot("", session.AppName),
