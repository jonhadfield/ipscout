...
// upstream applebot fixture, plus a real temp cache, with UseTestData off.
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/applebot.json")
	require.NoError(t, err)
...
func TestInitialiseEmptyDocumentNotCached(t *testing.T) {
	...
	body := []byte(`{"creationTime":"2023-10-27T10:00:00.000000","prefixes":[]}`)
	c := newMockedClient(t, http.StatusOK, body)
