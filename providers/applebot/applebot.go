...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/applebot"
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating applebot client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.Applebot.Enabled != nil && *c.Providers.Applebot.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.Applebot.OutputPriority
}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in applebot data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.Applebot.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is Applebot"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling applebot data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching applebot data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("applebot document contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling applebot provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.Applebot.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.Applebot.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting applebot data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising applebot client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking applebot cache: %w", err)
	}

	if ok {
		c.Logger.Debug("applebot provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading applebot provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading applebot api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading applebot provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached applebot provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading applebot cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/applebot/testdata/applebot_17_241_208_165_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting applebot test data file path: %w", err)
	}
	...
	c.Logger.Info("applebot match returned from test data", "host", "17.241.208.165")
...
// FindHost searches for the host in the applebot data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading applebot host data from cache: %w", err)
	}
	...
	case c.Host.Is4():
		for _, record := range doc.IPv4Prefixes {
			if record.IPv4Prefix.Contains(c.Host) {
				...
				c.Logger.Debug("returning applebot host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("Applebot | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("Applebot | Host: %s", "17.241.208.165")
	}
