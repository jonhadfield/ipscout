...
// upstream BGPView fixture (the first source vultr's Fetch queries), plus a
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	c := newMockedClient(t, "5.102.113.10", http.StatusOK, body)
	...
	require.Equal(t, netip.MustParsePrefix("5.102.113.0/24"), parsed.Prefix)
}
