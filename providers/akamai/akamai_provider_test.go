...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Akamai.Enabled = &enabled
	...
	pc.Providers.Akamai.Enabled = nil
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/akamai_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix("192.0.2.0/24"), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["HostedInAkamai"])
}
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"akamai":{"defaultMatchScore":5.0}}}`

	result, err := pc.RateHostData(data, []byte(ratingConfigJSON))
	require.NoError(t, err)
	require.True(t, result.Detected)
	require.InEpsilon(t, 5.0, result.Score, 0.0001)
	require.Equal(t, []string{"hosted in Akamai"}, result.Reasons)
}
...
func seedCache(t *testing.T, c *ProviderClient, doc ProviderDoc) {
	t.Helper()
...
func TestInitialiseAndFindHostFromCache(t *testing.T) {
	...
	seedCache(t, c, ProviderDoc{IPv4Prefixes: []netip.Prefix{netip.MustParsePrefix("192.0.2.0/24")}})
...
func TestFindHostIPv6FromCache(t *testing.T) {
	...
	seedCache(t, c, ProviderDoc{IPv6Prefixes: []netip.Prefix{netip.MustParsePrefix("2001:db8::/32")}})
