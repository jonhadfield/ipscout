...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/uptrends"
...
const (
	...
	// Uptrends publishes its checkpoint ranges as a static allowlist document.
	// A day, matching the other monitoring providers.
	DocTTL = 24 * time.Hour
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating uptrends client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.Uptrends.Enabled != nil && *c.Providers.Uptrends.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.Uptrends.OutputPriority
}
...
func (c *ProviderClient) ExtractThreatIndicators(findRes []byte) (*providers.ThreatIndicators, error) {
	...
	if doc.Prefix.IsValid() {
		indicators["UptrendsMonitor"] = "true"
	}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in uptrends data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.Uptrends.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is Uptrends monitoring"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling uptrends data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching uptrends data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("uptrends data contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling uptrends provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.Uptrends.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.Uptrends.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting uptrends data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising uptrends client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking uptrends cache: %w", err)
	}

	if ok {
		c.Logger.Debug("uptrends provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading uptrends provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading uptrends api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading uptrends provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached uptrends provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading uptrends cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/uptrends/testdata/uptrends_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting uptrends test data file path: %w", err)
	}
	...
	c.Logger.Info("uptrends match returned from test data", "host", testDataHost)
...
// FindHost searches for the host in the uptrends data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading uptrends host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning uptrends host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("Uptrends | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("Uptrends | Host: %s", testDataHost)
	}
