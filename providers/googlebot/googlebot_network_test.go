...
// upstream common-crawlers fixture, plus a real temp cache, with UseTestData off.
func newMockedClient(t *testing.T, status int, body []byte) *ProviderClient {
	...
	sess.Host = netip.MustParseAddr("66.249.77.135")
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/common-crawlers.json")
	require.NoError(t, err)
	...
	require.Equal(t, netip.MustParsePrefix("66.249.77.0/24"), parsed.Prefix)
}
