...
// upstream bingbot.json fixture, plus a real temp cache, with UseTestData off.
func newMockedClient(t *testing.T, status int, body []byte) *ProviderClient {
	...
	sess.Host = netip.MustParseAddr("157.55.39.10")
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/bingbot.json")
	require.NoError(t, err)
