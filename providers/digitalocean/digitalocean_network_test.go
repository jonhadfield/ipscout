...
// upstream google.csv fixture, plus a real temp cache, with UseTestData off.
func newMockedClient(t *testing.T, status int, body []byte) *ProviderClient {
	...
	sess.Host = netip.MustParseAddr("5.101.96.5")
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/google.csv")
	require.NoError(t, err)
	...
	require.Equal(t, netip.MustParsePrefix("5.101.96.0/21"), parsed.Record.Network)
	require.Equal(t, "NL", parsed.Record.CountryCode)
}
