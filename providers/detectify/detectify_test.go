...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Detectify.Enabled = &enabled
	...
	pc.Providers.Detectify.Enabled = nil
...
func TestPriority(t *testing.T) {
	...
	priority := int32(20)
	pc.Providers.Detectify.OutputPriority = &priority
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/detectify_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefixV4), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["DetectifyScanner"])
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"detectify":{"defaultMatchScore":10.0}}}`
	...
	require.Equal(t, []string{"source is a Detectify scanner"}, result.Reasons)
}
...
func TestRateHostDataNoMatch(t *testing.T) {
	...
	ratingConfigJSON := []byte(`{"providers":{"detectify":{"defaultMatchScore":10.0}}}`)
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte("<code>192.0.2.1</code><code>2001:db8::1</code>"))
...
func TestInitialiseNetworkEmptyDocument(t *testing.T) {
	...
	// A page carrying no <code> blocks fails inside ip-fetcher's own parser,
	// before the empty-document guard in loadProviderData is reached, so this
	// asserts the failure rather than that specific sentinel.
	c := newMockedClient(t, http.StatusOK, []byte("<html><body>no addresses here</body></html>"))

	err := c.Initialise()
	require.Error(t, err)
	require.Contains(t, err.Error(), "detectify")
}
