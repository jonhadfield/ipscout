...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Grafana.Enabled = &enabled
	...
	pc.Providers.Grafana.Enabled = nil
...
func TestPriority(t *testing.T) {
	...
	priority := int32(20)
	pc.Providers.Grafana.OutputPriority = &priority
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/grafana_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefixV4), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["GrafanaMonitor"])
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"grafana":{"defaultMatchScore":10.0}}}`
	...
	require.Equal(t, []string{"source is Grafana synthetic monitoring"}, result.Reasons)
}
...
func TestRateHostDataNoMatch(t *testing.T) {
	...
	ratingConfigJSON := []byte(`{"providers":{"grafana":{"defaultMatchScore":10.0}}}`)
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte("{\"all\":{\"ipv4\":[\"192.0.2.1/32\"],\"ipv6\":[\"2001:db8::1/128\"]},\"locations\":{\"lon\":{\"ipv4\":[\"192.0.2.1/32\"],\"ipv6\":[\"2001:db8::1/128\"]}}}"))
...
func TestInitialiseNetworkEmptyDocument(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte("{\"all\":{\"ipv4\":[],\"ipv6\":[]},\"locations\":{}}"))
