...
// real cache-miss -> API -> cache path offline.
func newMockedClient(t *testing.T, status int, body []byte) *Client {
	...
	sess.Providers.AbuseIPDB.APIKey = "test-key"
...
func TestFindHostNetworkSuccess(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/abuseipdb_194_169_175_35_report.json")
	require.NoError(t, err)
...
func TestFindHostNetworkUnauthorizedIsKeyRejected(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusUnauthorized, []byte(`{"errors":[{"detail":"Authentication failed.","status":401}]}`))
