...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/blocklistde"
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating blocklistde client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.BlocklistDE.Enabled != nil && *c.Providers.BlocklistDE.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.BlocklistDE.OutputPriority
}
...
func (c *ProviderClient) ExtractThreatIndicators(findRes []byte) (*providers.ThreatIndicators, error) {
	...
	if doc.Prefix.IsValid() {
		indicators["BlocklistDEListed"] = "true"
	}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in blocklistde data")
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling blocklistde data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching blocklistde data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("blocklistde data contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling blocklistde provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.BlocklistDE.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.BlocklistDE.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting blocklistde data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising blocklistde client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking blocklistde cache: %w", err)
	}

	if ok {
		c.Logger.Debug("blocklistde provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading blocklistde provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading blocklistde api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading blocklistde provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached blocklistde provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading blocklistde cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/blocklistde/testdata/blocklistde_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting blocklistde test data file path: %w", err)
	}
...
// FindHost searches for the host in the blocklistde data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading blocklistde host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning blocklistde host match data")
