...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.BlocklistDE.Enabled = &enabled
	...
	pc.Providers.BlocklistDE.Enabled = nil
...
func TestPriority(t *testing.T) {
	...
	priority := int32(20)
	pc.Providers.BlocklistDE.OutputPriority = &priority
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/blocklistde_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefixV4), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["BlocklistDEListed"])
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"blocklistde":{"defaultMatchScore":10.0}}}`

	result, err := pc.RateHostData(data, []byte(ratingConfigJSON))
	require.NoError(t, err)
	require.True(t, result.Detected)
	require.InEpsilon(t, 10.0, result.Score, 0.0001)
	require.Equal(t, "high", result.Threat)
	require.Equal(t, []string{"listed on Blocklist.de"}, result.Reasons)
}
...
func TestRateHostDataNoMatch(t *testing.T) {
	...
	ratingConfigJSON := []byte(`{"providers":{"blocklistde":{"defaultMatchScore":10.0}}}`)
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	// upstream serves a newline separated list of bare addresses
	c := newMockedClient(t, http.StatusOK, []byte("192.0.2.1\n2001:db8::1\n"))
