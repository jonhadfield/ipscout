...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.StatusCake.Enabled = &enabled
	...
	pc.Providers.StatusCake.Enabled = nil
...
func TestPriority(t *testing.T) {
	...
	priority := int32(20)
	pc.Providers.StatusCake.OutputPriority = &priority
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/statuscake_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefixV4), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["StatusCakeMonitor"])
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"statuscake":{"defaultMatchScore":10.0}}}`
	...
	require.Equal(t, []string{"source is StatusCake monitoring"}, result.Reasons)
}
...
func TestRateHostDataNoMatch(t *testing.T) {
	...
	ratingConfigJSON := []byte(`{"providers":{"statuscake":{"defaultMatchScore":10.0}}}`)
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte("{\"1\": {\"guid\": \"1\", \"servercode\": \"T1\", \"title\": \"Test\", \"ip\": \"192.0.2.1\", \"ipv6\": \"2001:db8::1\", \"countryiso\": \"GBR\", \"status\": \"Up\"}}"))
