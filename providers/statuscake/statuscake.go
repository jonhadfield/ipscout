...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/statuscake"
...
const (
	...
	// StatusCake serves the locations endpoint with Cache-Control: no-store and
	// no Last-Modified at all, so the headers say nothing. The content was
	// unchanged over a five minute resample. A day until there is a longer
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating statuscake client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.StatusCake.Enabled != nil && *c.Providers.StatusCake.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.StatusCake.OutputPriority
}
...
func (c *ProviderClient) ExtractThreatIndicators(findRes []byte) (*providers.ThreatIndicators, error) {
	...
	if doc.Prefix.IsValid() {
		indicators["StatusCakeMonitor"] = "true"
	}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in statuscake data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.StatusCake.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is StatusCake monitoring"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling statuscake data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching statuscake data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("statuscake data contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling statuscake provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.StatusCake.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.StatusCake.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting statuscake data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising statuscake client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking statuscake cache: %w", err)
	}

	if ok {
		c.Logger.Debug("statuscake provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading statuscake provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading statuscake api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading statuscake provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached statuscake provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading statuscake cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/statuscake/testdata/statuscake_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting statuscake test data file path: %w", err)
	}
	...
	c.Logger.Info("statuscake match returned from test data", "host", testDataHost)
...
// FindHost searches for the host in the statuscake data
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("StatusCake | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("StatusCake | Host: %s", testDataHost)
	}
