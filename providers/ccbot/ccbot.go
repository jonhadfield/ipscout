...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating ccbot client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.CCBot.Enabled != nil && *c.Providers.CCBot.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.CCBot.OutputPriority
}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in ccbot data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.CCBot.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is CCBot"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling ccbot data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching ccbot data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("ccbot document contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling ccbot provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.CCBot.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.CCBot.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting ccbot data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising ccbot client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking ccbot cache: %w", err)
	}

	if ok {
		c.Logger.Debug("ccbot provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading ccbot provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading ccbot api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading ccbot provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached ccbot provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading ccbot cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/ccbot/testdata/ccbot_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting ccbot test data file path: %w", err)
	}
	...
	c.Logger.Info("ccbot match returned from test data", "host", "192.0.2.1")
...
// FindHost searches for the host in the ccbot data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading ccbot host data from cache: %w", err)
	}
	...
	case c.Host.Is4():
		for _, record := range doc.IPv4Prefixes {
			if record.IPv4Prefix.Contains(c.Host) {
				...
				c.Logger.Debug("returning ccbot host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("CCBot | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("CCBot | Host: %s", "192.0.2.1")
	}
