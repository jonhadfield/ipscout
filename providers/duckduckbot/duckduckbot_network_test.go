...
// upstream duckduckbot fixture, plus a real temp cache, with UseTestData off.
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/duckduckbot.json")
	require.NoError(t, err)
...
func TestInitialiseEmptyDocumentNotCached(t *testing.T) {
	...
	body := []byte(`{"creationTime":"2026-07-03T15:15:37.000000","prefixes":[]}`)
	c := newMockedClient(t, http.StatusOK, body)
