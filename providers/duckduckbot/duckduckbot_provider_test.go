...
import (
	...
	fetcherDuckduckbot "github.com/jonhadfield/ip-fetcher/providers/duckduckbot"
...
// Values that match providers/duckduckbot/testdata/duckduckbot_104_43_54_127_report.json.
const (
	testHost         = "104.43.54.127"
	testPrefix       = "104.43.54.127/32"
	testDefaultScore = 7.5
	testCreationRFC  = "2026-07-03T15:15:37Z"
)
...
// seedCache writes a duckduckbot provider document into the cache so that the
// non-test-data code paths (Initialise hit, loadProviderDataFromCache, FindHost
// search) can run without any network access. The live feed is currently
// IPv4-only, so a documentation-range IPv6 prefix exercises the IPv6 path.
func seedCache(t *testing.T, c *ProviderClient) {
	t.Helper()

	doc := fetcherDuckduckbot.Doc{
		CreationTime: time.Date(2026, 7, 3, 15, 15, 37, 0, time.UTC),
		IPv4Prefixes: []fetcherDuckduckbot.IPv4Entry{
			{IPv4Prefix: netip.MustParsePrefix(testPrefix)},
		},
		IPv6Prefixes: []fetcherDuckduckbot.IPv6Entry{
			{IPv6Prefix: netip.MustParsePrefix("2001:db8:4860::/64")},
		},
	}
...
func TestEnabled(t *testing.T) {
	...
	enabled := true
	pc.Providers.DuckDuckBot.Enabled = &enabled
...
func TestPriority(t *testing.T) {
	...
	priority := int32(9)
	pc.Providers.DuckDuckBot.OutputPriority = &priority
...
func TestFindHostFromCacheIPv6(t *testing.T) {
	...
	c := newTestProviderClient(t)
	c.UseTestData = false
	c.Host = netip.MustParseAddr("2001:db8:4860::1")
	seedCache(t, c)
	...
	parsed, err := unmarshalResponse(res)
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix("2001:db8:4860::/64"), parsed.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["DuckDuckBot"])
}
...
func TestRateHostData(t *testing.T) {
	...
	require.Equal(t, []string{"source is DuckDuckBot"}, result.Reasons)
}
...
func TestRateHostDataNoPrefix(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"duckduckbot":{"defaultMatchScore":7.5}}}`
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/duckduckbot_104_43_54_127_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefix), res.Prefix)
	require.False(t, res.CreationTime.IsZero())
}
