...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating googlesc client")
...
func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.GoogleSC.Enabled != nil && *c.Providers.GoogleSC.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.GoogleSC.OutputPriority
}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if err := json.Unmarshal(findRes, &doc); err != nil {
		return providers.RateResult{}, fmt.Errorf("error unmarshalling GoogleSC find result: %w", err)
	}
	...
	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.GoogleSC.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is GoogleSC"}
	}
...
func unmarshalProviderData(data []byte) (*googlesc.Doc, error) {
	var res *googlesc.Doc

	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling googlesc data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	googlescClient := googlesc.New()
	googlescClient.Client = c.HTTPClient

	if c.Providers.GoogleSC.URL != "" {
		googlescClient.DownloadURL = c.Providers.GoogleSC.URL
		c.Logger.Debug("overriding googlesc source", "url", googlescClient.DownloadURL)
	}

	doc, err := googlescClient.Fetch()
	if err != nil {
		return fmt.Errorf("error fetching googlesc data: %w", err)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling googlesc provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.GoogleSC.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.GoogleSC.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting googlesc data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	if err != nil {
		return fmt.Errorf("checking googlesc cache: %w", err)
	}

	if ok {
		c.Logger.Debug("googlesc provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading googlesc provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading googlesc api response: %w", err)
	}

	return nil
}

func (c *ProviderClient) loadProviderDataFromCache() (*googlesc.Doc, error) {
	c.Logger.Debug("loading googlesc provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached googlesc provider doc: %w", uErr)
		}
	} else {
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/googlesc/testdata/googlesc_74_125_219_32_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting google special crawlers test data file path: %w", err)
	}
	...
	c.Logger.Info("googlesc match returned from test data", "host", "74.125.219.32")
...
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading googlesc host data from cache: %w", err)
	}
	...
	case c.Host.Is4():
		for _, record := range doc.IPv4Prefixes {
			if record.IPv4Prefix.Contains(c.Host) {
				...
				c.Logger.Debug("returning googlesc host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	if c.UseTestData {
		tw.SetTitle("GOOGLE SPECIAL CRAWLER | Host: %s", "74.125.219.32")
	}
