...
// upstream special-crawlers fixture, plus a real temp cache, with UseTestData off.
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/special-crawlers.json")
	require.NoError(t, err)
	...
	require.Equal(t, netip.MustParsePrefix("74.125.219.0/24"), parsed.Prefix)
}
