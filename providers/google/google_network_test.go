...
// upstream goog.json fixture, plus a real temp cache, with UseTestData off.
func newMockedClient(t *testing.T, status int, body []byte) *ProviderClient {
	...
	sess.Host = netip.MustParseAddr("34.3.8.8")
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/goog.json")
	require.NoError(t, err)
	...
	require.Equal(t, netip.MustParsePrefix("34.3.8.0/24"), parsed.Prefix)
}
