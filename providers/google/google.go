...
func (c *ProviderClient) Initialise() error {
	...
	if ok {
		c.Logger.Debug("google provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading google provider data from source")
...
func (c *ProviderClient) loadProviderDataFromCache() (*google.Doc, error) {
	c.Logger.Debug("loading google provider data from cache")
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/google/testdata/google_34_3_8_8_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting google test data file path: %w", err)
	}
