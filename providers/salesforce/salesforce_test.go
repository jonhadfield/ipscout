...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Salesforce.Enabled = &enabled
	...
	pc.Providers.Salesforce.Enabled = nil
...
func TestUnmarshalResponse(t *testing.T) {
	...
	data := []byte(`{"prefix":"192.0.2.0/24","region":"us-west-2","provider":"aws"}`)
	res, err := unmarshalResponse(data)
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix("192.0.2.0/24"), res.Prefix)
	require.Equal(t, "us-west-2", res.Region)
	require.Equal(t, "aws", res.Provider)
...
func TestUnmarshalProviderData(t *testing.T) {
	...
	require.Equal(t, "us-west-2", res.Regions[0].Name)
	require.Equal(t, "aws", res.Regions[0].Provider)
}

func TestLoadResultsFile(t *testing.T) {
	t.Parallel()

	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/salesforce_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix("192.0.2.0/24"), res.Prefix)
	require.Equal(t, "us-west-2", res.Region)
	require.Equal(t, "aws", res.Provider)
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["HostedInSalesforce"])
}
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"salesforce":{"defaultMatchScore":5.0}}}`

	result, err := pc.RateHostData(data, []byte(ratingConfigJSON))
	require.NoError(t, err)
	require.True(t, result.Detected)
	require.InEpsilon(t, 5.0, result.Score, 0.0001)
	require.Equal(t, []string{"hosted in Salesforce"}, result.Reasons)
}
...
func TestInitialiseAndFindHostFromCache(t *testing.T) {
	...
	require.Equal(t, "us-west-2", parsed.Region)
	require.Equal(t, "aws", parsed.Provider)
