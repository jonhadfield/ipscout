...
const (
	...
	// bogons are published as network prefixes rather than single addresses,
	// so the checked-in report carries the containing /24
	testDataPrefix = "192.0.2.0/24"
)
...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Cymru.Enabled = &enabled
	...
	pc.Providers.Cymru.Enabled = nil
...
func TestPriority(t *testing.T) {
	...
	priority := int32(20)
	pc.Providers.Cymru.OutputPriority = &priority
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/cymru_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testDataPrefix), res.Prefix)
	require.False(t, res.LastUpdated.IsZero())
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["CymruBogon"])
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"cymru":{"defaultMatchScore":10.0}}}`
	...
	require.Equal(t, []string{"listed in the Team Cymru full bogon list (unallocated or unroutable space)"}, result.Reasons)
}
...
func TestRateHostDataNoMatch(t *testing.T) {
	...
	ratingConfigJSON := []byte(`{"providers":{"cymru":{"defaultMatchScore":10.0}}}`)
...
func TestFindHostUsesTestData(t *testing.T) {
	...
	var parsed HostSearchResult
	require.NoError(t, json.Unmarshal(res, &parsed))
	require.Equal(t, netip.MustParsePrefix(testDataPrefix), parsed.Prefix)
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	// upstream serves a commented header carrying the generation time, then one
	// CIDR per line. The mock returns this for both the v4 and v6 lists.
	c := newMockedClient(t, http.StatusOK, []byte("# last updated 1787993701 (Sat Aug 29 08:55:01 2026 GMT)\n192.0.2.0/24\n2001:db8::/32\n"))
	...
	require.NoError(t, json.Unmarshal(res, &parsed))
	require.Equal(t, netip.MustParsePrefix(testDataPrefix), parsed.Prefix)
	// the header timestamp is carried through to the result
	require.False(t, parsed.LastUpdated.IsZero())
}
...
func TestInitialiseNetworkEmptyDocument(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte("# no entries\n"))
