...
// upstream ip-ranges fixture, plus a real temp cache, with UseTestData off.
func newMockedClient(t *testing.T, status int, body []byte) *ProviderClient {
	...
	sess.Host = netip.MustParseAddr("18.164.52.75")
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/ip-ranges.json")
	require.NoError(t, err)
	...
	require.Equal(t, netip.MustParsePrefix("18.164.0.0/15"), parsed.IPPrefix)
	require.Equal(t, "AMAZON", parsed.Prefix.Service)
}
