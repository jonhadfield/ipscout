...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/googleutf"
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating googleutf client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.GoogleUTF.Enabled != nil && *c.Providers.GoogleUTF.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.GoogleUTF.OutputPriority
}
...
func (c *ProviderClient) ExtractThreatIndicators(findRes []byte) (*providers.ThreatIndicators, error) {
	...
	if doc.Prefix.IsValid() {
		indicators["GoogleUserTriggeredFetcher"] = "true"
	}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in googleutf data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.GoogleUTF.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is Google User-triggered Fetcher"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling googleutf data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching googleutf data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("googleutf document contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling googleutf provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.GoogleUTF.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.GoogleUTF.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting googleutf data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising googleutf client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking googleutf cache: %w", err)
	}

	if ok {
		c.Logger.Debug("googleutf provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading googleutf provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading googleutf api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading googleutf provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached googleutf provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading googleutf cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/googleutf/testdata/googleutf_34_100_182_100_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting googleutf test data file path: %w", err)
	}
	...
	c.Logger.Info("googleutf match returned from test data", "host", "34.100.182.100")
...
// FindHost searches for the host in the googleutf data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading googleutf host data from cache: %w", err)
	}
	...
	case c.Host.Is4():
		for _, record := range doc.IPv4Prefixes {
			if record.IPv4Prefix.Contains(c.Host) {
				...
				c.Logger.Debug("returning googleutf host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("Google User-triggered Fetchers | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("Google User-triggered Fetchers | Host: %s", "34.100.182.100")
	}
