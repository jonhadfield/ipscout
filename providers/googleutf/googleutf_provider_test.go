...
import (
	...
	fetcherGoogleutf "github.com/jonhadfield/ip-fetcher/providers/googleutf"
...
// Values that match providers/googleutf/testdata/googleutf_34_100_182_100_report.json.
const (
	testHost         = "34.100.182.100"
	testPrefix       = "34.100.182.96/28"
...
// seedCache writes a googleutf provider document into the cache so that the
...
func seedCache(t *testing.T, c *ProviderClient) {
	...
	doc := fetcherGoogleutf.Doc{
		CreationTime: time.Date(2024, 11, 14, 0, 0, 0, 0, time.UTC),
		IPv4Prefixes: []fetcherGoogleutf.IPv4Entry{
			{IPv4Prefix: netip.MustParsePrefix(testPrefix)},
		},
		IPv6Prefixes: []fetcherGoogleutf.IPv6Entry{
			{IPv6Prefix: netip.MustParsePrefix("2001:4860:4801:2008::/64")},
		},
	}
...
func TestEnabled(t *testing.T) {
	...
	enabled := true
	pc.Providers.GoogleUTF.Enabled = &enabled
...
func TestPriority(t *testing.T) {
	...
	priority := int32(9)
	pc.Providers.GoogleUTF.OutputPriority = &priority
...
func TestFindHostFromCacheIPv6(t *testing.T) {
	...
	c := newTestProviderClient(t)
	c.UseTestData = false
	c.Host = netip.MustParseAddr("2001:4860:4801:2008::1")
	seedCache(t, c)
	...
	parsed, err := unmarshalResponse(res)
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix("2001:4860:4801:2008::/64"), parsed.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["GoogleUserTriggeredFetcher"])
}
...
func TestRateHostData(t *testing.T) {
	...
	require.Equal(t, []string{"source is Google User-triggered Fetcher"}, result.Reasons)
}
...
func TestRateHostDataNoPrefix(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"googleutf":{"defaultMatchScore":7.5}}}`
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/googleutf_34_100_182_100_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefix), res.Prefix)
	require.False(t, res.CreationTime.IsZero())
}
