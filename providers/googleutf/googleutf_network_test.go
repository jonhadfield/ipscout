...
// upstream user-triggered-fetchers fixture, plus a real temp cache, with UseTestData off.
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/user-triggered-fetchers.json")
	require.NoError(t, err)
...
func TestInitialiseNetworkFetchError(t *testing.T) {
	...
	// ip-fetcher v0.0.18 never rejects on HTTP status, so an upstream error
...
func TestInitialiseEmptyDocumentNotCached(t *testing.T) {
	...
	body := []byte(`{"creationTime":"2024-11-14T00:00:00.000000","prefixes":[]}`)
	c := newMockedClient(t, http.StatusOK, body)
