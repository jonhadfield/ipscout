...
func (c *ProviderClient) Initialise() error {
	...
	if ok {
		c.Logger.Debug("gcp provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading gcp provider data from source")
...
func (c *ProviderClient) loadProviderDataFromCache() (*gcp.Doc, error) {
	c.Logger.Debug("loading gcp provider data from cache")
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/gcp/testdata/gcp_34_128_62_0_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting GCP test data file path: %w", err)
	}
