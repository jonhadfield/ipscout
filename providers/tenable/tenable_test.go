...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Tenable.Enabled = &enabled
	...
	pc.Providers.Tenable.Enabled = nil
...
func TestPriority(t *testing.T) {
	...
	priority := int32(20)
	pc.Providers.Tenable.OutputPriority = &priority
...
func TestUnmarshalProviderData(t *testing.T) {
	...
	doc := ipfetcher.Doc{
		Prefixes:     []ipfetcher.Prefix{{IPPrefix: netip.MustParsePrefix(testPrefixV4)}},
		IPv6Prefixes: []ipfetcher.Prefix{{IPPrefix: netip.MustParsePrefix(testPrefixV6)}},
	}
	...
	require.NoError(t, err)
	require.Equal(t, doc.Prefixes[0], res.Prefixes[0])
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/tenable_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefixV4), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["TenableScanner"])
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"tenable":{"defaultMatchScore":10.0}}}`
	...
	require.Equal(t, []string{"source is a Tenable cloud scanner"}, result.Reasons)
}
...
func TestRateHostDataNoMatch(t *testing.T) {
	...
	ratingConfigJSON := []byte(`{"providers":{"tenable":{"defaultMatchScore":10.0}}}`)
...
func TestInitialiseAndFindHostFromCache(t *testing.T) {
	...
	seedCache(t, c, ipfetcher.Doc{Prefixes: []ipfetcher.Prefix{{IPPrefix: netip.MustParsePrefix(testPrefixV4)}}})
...
func TestFindHostIPv6FromCache(t *testing.T) {
	...
	seedCache(t, c, ipfetcher.Doc{IPv6Prefixes: []ipfetcher.Prefix{{IPPrefix: netip.MustParsePrefix(testPrefixV6)}}})
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte("{\"syncToken\":\"1\",\"createDate\":\"2026-09-01\",\"prefixes\":[{\"ip_prefix\":\"192.0.2.1/32\",\"region\":\"us-east-1\",\"service\":\"TENABLE_IO\",\"sensor_group\":\"cloud-scanner\"}],\"ipv6_prefixes\":[{\"ip_prefix\":\"2001:db8::1/128\"}]}"))
...
func TestInitialiseNetworkEmptyDocument(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte("{\"syncToken\":\"1\",\"createDate\":\"2026-09-01\",\"prefixes\":[],\"ipv6_prefixes\":[]}"))
