...
const (
	...
	testServiceArea = "Exchange Online"
	testCategory    = "Optimize"
)
...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.M365.Enabled = &enabled
...
func TestPriority(t *testing.T) {
	...
	priority := int32(20)
	pc.Providers.M365.OutputPriority = &priority
...
func TestUnmarshalProviderData(t *testing.T) {
	...
	doc := ipfetcher.Doc{
		Endpoints: []ipfetcher.Endpoint{{ServiceAreaDisplayName: testServiceArea, Category: testCategory, IPv4Prefixes: []netip.Prefix{netip.MustParsePrefix(testPrefixV4)}, IPv6Prefixes: []netip.Prefix{netip.MustParsePrefix(testPrefixV6)}}},
	}
	...
	require.NoError(t, err)
	require.Equal(t, doc.Endpoints[0].IPv4Prefixes[0], res.Endpoints[0].IPv4Prefixes[0])
	require.Equal(t, doc.Endpoints[0].IPv6Prefixes[0], res.Endpoints[0].IPv6Prefixes[0])
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/m365_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefixV4), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["Microsoft365"])
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"m365":{"defaultMatchScore":10.0}}}`
	...
	require.Equal(t, []string{"source is Microsoft 365 infrastructure"}, result.Reasons)
}
...
func TestRateHostDataNoMatch(t *testing.T) {
	...
	ratingConfigJSON := []byte(`{"providers":{"m365":{"defaultMatchScore":10.0}}}`)
...
func TestInitialiseAndFindHostFromCache(t *testing.T) {
	...
	seedCache(t, c, ipfetcher.Doc{Endpoints: []ipfetcher.Endpoint{{ServiceAreaDisplayName: testServiceArea, Category: testCategory, IPv4Prefixes: []netip.Prefix{netip.MustParsePrefix(testPrefixV4)}}}})
...
func TestFindHostIPv6FromCache(t *testing.T) {
	...
	seedCache(t, c, ipfetcher.Doc{Endpoints: []ipfetcher.Endpoint{{ServiceAreaDisplayName: testServiceArea, Category: testCategory, IPv6Prefixes: []netip.Prefix{netip.MustParsePrefix(testPrefixV6)}}}})
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte("[{\"id\":1,\"serviceArea\":\"Exchange\",\"serviceAreaDisplayName\":\"Exchange Online\",\"category\":\"Optimize\",\"ips\":[\"192.0.2.1/32\",\"2001:db8::1/128\"]}]"))
