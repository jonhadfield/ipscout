...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/tor"
...
const (
	ProviderName = "tor"
	// The exit list turns over as relays come and go, and a stale list both misses
	// new exits and flags addresses that have stopped being one. An hour.
	DocTTL = time.Hour
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating tor client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.Tor.Enabled != nil && *c.Providers.Tor.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.Tor.OutputPriority
}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in tor data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.Tor.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is a Tor exit node"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling tor data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching tor data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("tor data contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling tor provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.Tor.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.Tor.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting tor data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising tor client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking tor cache: %w", err)
	}

	if ok {
		c.Logger.Debug("tor provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading tor provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading tor api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading tor provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached tor provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading tor cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/tor/testdata/tor_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting tor test data file path: %w", err)
	}
	...
	c.Logger.Info("tor match returned from test data", "host", testDataHost)
...
// FindHost searches for the host in the tor data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading tor host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning tor host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("Tor Exit Node | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("Tor Exit Node | Host: %s", testDataHost)
	}
