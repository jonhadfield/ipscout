...
const testFixtureHost = "183.81.169.238"
...
func newMockedClient(t *testing.T, status int, body []byte) *ProviderClient {
	...
	sess.Host = netip.MustParseAddr(testFixtureHost)

	enabled := true
	sess.Providers.VirusTotal.Enabled = &enabled
	sess.Providers.VirusTotal.APIKey = "test-key"
...
func TestFindHostNetworkSuccess(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/virustotal_183_81_169_238_resp.json")
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, testFixtureHost, parsed.Data.ID)
	require.Equal(t, "NL", parsed.Data.Attributes.Country)
...
func TestFindHostNetworkUnauthorizedIsKeyRejected(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusUnauthorized, []byte(`{"error": {"code": "WrongCredentialsError", "message": "Wrong API key"}}`))
