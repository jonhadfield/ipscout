...
const (
	...
	// Site24x7 publishes its monitoring location list without a refresh interval.
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating site24x7 client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.Site24x7.Enabled != nil && *c.Providers.Site24x7.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.Site24x7.OutputPriority
}
...
func (c *ProviderClient) ExtractThreatIndicators(findRes []byte) (*providers.ThreatIndicators, error) {
	...
	if doc.Prefix.IsValid() {
		indicators["Site24x7Monitor"] = "true"
	}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in site24x7 data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.Site24x7.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is Site24x7 monitoring"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling site24x7 data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching site24x7 data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("site24x7 data contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling site24x7 provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.Site24x7.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.Site24x7.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting site24x7 data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising site24x7 client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking site24x7 cache: %w", err)
	}

	if ok {
		c.Logger.Debug("site24x7 provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading site24x7 provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading site24x7 api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading site24x7 provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached site24x7 provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading site24x7 cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/site24x7/testdata/site24x7_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting site24x7 test data file path: %w", err)
	}
	...
	c.Logger.Info("site24x7 match returned from test data", "host", testDataHost)
...
// FindHost searches for the host in the site24x7 data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading site24x7 host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning site24x7 host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("Site24x7 | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("Site24x7 | Host: %s", testDataHost)
	}
