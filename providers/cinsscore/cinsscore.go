...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating cinsscore client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.CINSScore.Enabled != nil && *c.Providers.CINSScore.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.CINSScore.OutputPriority
}
...
func (c *ProviderClient) ExtractThreatIndicators(findRes []byte) (*providers.ThreatIndicators, error) {
	...
	if doc.Prefix.IsValid() {
		indicators["ListedOnCINSArmyList"] = "true"
	}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in cinsscore data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.CINSScore.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"listed on the CINS Army List"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling cinsscore data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching cinsscore data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("cinsscore document contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling cinsscore provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.CINSScore.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.CINSScore.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting cinsscore data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising cinsscore client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking cinsscore cache: %w", err)
	}

	if ok {
		c.Logger.Debug("cinsscore provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading cinsscore provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading cinsscore api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading cinsscore provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached cinsscore provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading cinsscore cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/cinsscore/testdata/cinsscore_198_51_100_42_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting cinsscore test data file path: %w", err)
	}
	...
	c.Logger.Info("cinsscore match returned from test data", "host", testDataHost)
...
// FindHost searches for the host in the cinsscore data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning cinsscore host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("CINS Army List | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("CINS Army List | Host: %s", testDataHost)
	}
