...
// upstream BGPView fixture (the first source ovh's Fetch queries), plus a
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/bgpview-prefixes.json")
	require.NoError(t, err)

	c := newMockedClient(t, "51.68.10.20", http.StatusOK, body)
	...
	require.Equal(t, netip.MustParsePrefix("51.68.0.0/16"), parsed.Prefix)
}

func TestInitialiseNetworkFetchError(t *testing.T) {
	t.Parallel()

	// A non-200 response makes ip-fetcher's Fetch fail (both BGPView and the
	// RIPE stat fallback see the same error), surfacing from loadProviderData.
	c := newMockedClient(t, "51.68.10.20", http.StatusInternalServerError, []byte("boom"))
