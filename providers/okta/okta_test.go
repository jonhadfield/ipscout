...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Okta.Enabled = &enabled
...
func TestPriority(t *testing.T) {
	...
	priority := int32(20)
	pc.Providers.Okta.OutputPriority = &priority
...
func TestUnmarshalProviderData(t *testing.T) {
	...
	doc := ipfetcher.Doc{
		Cells: []ipfetcher.Cell{{Name: testCell, IPv4Prefixes: []netip.Prefix{netip.MustParsePrefix(testPrefixV4)}, IPv6Prefixes: []netip.Prefix{netip.MustParsePrefix(testPrefixV6)}}},
	}
	...
	require.NoError(t, err)
	require.Equal(t, doc.Cells[0].IPv4Prefixes[0], res.Cells[0].IPv4Prefixes[0])
	require.Equal(t, doc.Cells[0].IPv6Prefixes[0], res.Cells[0].IPv6Prefixes[0])
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/okta_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefixV4), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["OktaInfrastructure"])
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"okta":{"defaultMatchScore":10.0}}}`
	...
	require.Equal(t, []string{"source is Okta infrastructure"}, result.Reasons)
}
...
func TestRateHostDataNoMatch(t *testing.T) {
	...
	ratingConfigJSON := []byte(`{"providers":{"okta":{"defaultMatchScore":10.0}}}`)
...
func TestInitialiseAndFindHostFromCache(t *testing.T) {
	...
	seedCache(t, c, ipfetcher.Doc{Cells: []ipfetcher.Cell{{Name: testCell, IPv4Prefixes: []netip.Prefix{netip.MustParsePrefix(testPrefixV4)}}}})
...
func TestFindHostIPv6FromCache(t *testing.T) {
	...
	seedCache(t, c, ipfetcher.Doc{Cells: []ipfetcher.Cell{{Name: testCell, IPv6Prefixes: []netip.Prefix{netip.MustParsePrefix(testPrefixV6)}}}})
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte("{\"us_cell1\":{\"ip_ranges\":[\"192.0.2.1/32\",\"2001:db8::1/128\"]}}"))
