...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/intercom"
...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Intercom.Enabled = &enabled
	...
	pc.Providers.Intercom.Enabled = nil
...
func TestUnmarshalResponse(t *testing.T) {
	...
	data := []byte(`{"prefix":"192.0.2.0/24","region":"US","service":"web"}`)
	...
	require.Equal(t, "web", res.Service)
...
func TestUnmarshalProviderData(t *testing.T) {
	...
	doc := ipfetcher.Doc{
		IPRanges: []ipfetcher.RawRange{
			{Range: "192.0.2.0/24", Region: "US", Service: "web"},
		},
	...
	require.Equal(t, "US", res.IPRanges[0].Region)
	require.Equal(t, "web", res.IPRanges[0].Service)
}

func TestLoadResultsFile(t *testing.T) {
	t.Parallel()

	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/intercom_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix("192.0.2.0/24"), res.Prefix)
	require.Equal(t, "US", res.Region)
	require.Equal(t, "web", res.Service)
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["HostedInIntercom"])
}
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"intercom":{"defaultMatchScore":5.0}}}`

	result, err := pc.RateHostData(data, []byte(ratingConfigJSON))
	require.NoError(t, err)
	require.True(t, result.Detected)
	require.InEpsilon(t, 5.0, result.Score, 0.0001)
	require.Equal(t, []string{"hosted in Intercom"}, result.Reasons)
}
...
func TestInitialiseAndFindHostFromCache(t *testing.T) {
	...
	require.Equal(t, "US", parsed.Region)
	require.Equal(t, "web", parsed.Service)
