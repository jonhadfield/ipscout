...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/feodo"
...
const (
	ProviderName = "feodo"
	// abuse.ch republishes the list frequently as hosts are taken down and new ones
	// appear. An hour keeps it current without refetching on every lookup.
	DocTTL = time.Hour
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating feodo client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.Feodo.Enabled != nil && *c.Providers.Feodo.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.Feodo.OutputPriority
}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in feodo data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.Feodo.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is a Feodo Tracker botnet command and control server"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling feodo data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching feodo data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("feodo data contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling feodo provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.Feodo.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.Feodo.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting feodo data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising feodo client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking feodo cache: %w", err)
	}

	if ok {
		c.Logger.Debug("feodo provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading feodo provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading feodo api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading feodo provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached feodo provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading feodo cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/feodo/testdata/feodo_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting feodo test data file path: %w", err)
	}
	...
	c.Logger.Info("feodo match returned from test data", "host", testDataHost)
...
// FindHost searches for the host in the feodo data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading feodo host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning feodo host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("Feodo Tracker | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("Feodo Tracker | Host: %s", testDataHost)
	}
