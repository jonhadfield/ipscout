...
func TestEnabled(t *testing.T) {
	...
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Updown.Enabled = &enabled
	...
	pc.Providers.Updown.Enabled = nil
...
func TestPriority(t *testing.T) {
	...
	priority := int32(20)
	pc.Providers.Updown.OutputPriority = &priority
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/updown_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefixV4), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["UpdownMonitor"])
...
func TestRateHostData(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"updown":{"defaultMatchScore":10.0}}}`
	...
	require.Equal(t, []string{"source is updown.io monitoring"}, result.Reasons)
}
...
func TestRateHostDataNoMatch(t *testing.T) {
	...
	ratingConfigJSON := []byte(`{"providers":{"updown":{"defaultMatchScore":10.0}}}`)
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte("{\"lon\":{\"ip\":\"192.0.2.1\",\"ip6\":\"2001:db8::1\",\"city\":\"London\",\"country\":\"UK\",\"country_code\":\"gb\",\"lat\":51.5,\"lng\":-0.1}}"))
