...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/updown"
...
const (
	ProviderName = "updown"
	// updown.io serves its node list from an API endpoint with no stated cadence.
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating updown client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.Updown.Enabled != nil && *c.Providers.Updown.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.Updown.OutputPriority
}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in updown data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.Updown.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is updown.io monitoring"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling updown data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching updown data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("updown data contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling updown provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.Updown.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.Updown.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting updown data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising updown client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking updown cache: %w", err)
	}

	if ok {
		c.Logger.Debug("updown provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading updown provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading updown api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading updown provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached updown provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading updown cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/updown/testdata/updown_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting updown test data file path: %w", err)
	}
	...
	c.Logger.Info("updown match returned from test data", "host", testDataHost)
...
// FindHost searches for the host in the updown data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading updown host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning updown host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("updown.io | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("updown.io | Host: %s", testDataHost)
	}
