...
// upstream BGPView fixture (the primary bgpview source), plus a real temp
...
func newMockedClient(t *testing.T, status int, body []byte) *ProviderClient {
	...
	sess.Host = netip.MustParseAddr("47.246.10.20")
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	// (BGPView first), served by the mock transport, then caches the parsed Doc.
	...
	require.Equal(t, netip.MustParsePrefix("47.246.0.0/16"), parsed.Prefix)
}

func TestInitialiseNetworkFetchError(t *testing.T) {
	t.Parallel()

	// A non-200 response makes both the RIPE stat and BGPView fetches fail,
	// surfacing from loadProviderData.
