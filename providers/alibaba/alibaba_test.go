...
func TestEnabled(t *testing.T) {
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Alibaba.Enabled = &enabled
	...
	pc.Providers.Alibaba.Enabled = nil
...
func TestLoadResultsFile(t *testing.T) {
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/alibaba_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix("192.0.2.0/24"), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["HostedInAlibaba"])
}
...
func TestRateHostData(t *testing.T) {
	...
	// Simple rating config JSON with just the alibaba section
	ratingConfigJSON := `{
		"providers": {
			"alibaba": {
	...
	require.Equal(t, []string{"hosted in Alibaba"}, result.Reasons)
}
