...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating uptimerobot client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.UptimeRobot.Enabled != nil && *c.Providers.UptimeRobot.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.UptimeRobot.OutputPriority
}
...
func (c *ProviderClient) ExtractThreatIndicators(findRes []byte) (*providers.ThreatIndicators, error) {
	...
	if doc.Prefix.IsValid() {
		indicators["UptimeRobotMonitor"] = "true"
	}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in uptimerobot data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.UptimeRobot.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is UptimeRobot monitoring"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling uptimerobot data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching uptimerobot data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("uptimerobot document contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling uptimerobot provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.UptimeRobot.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.UptimeRobot.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting uptimerobot data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising uptimerobot client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking uptimerobot cache: %w", err)
	}

	if ok {
		c.Logger.Debug("uptimerobot provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading uptimerobot provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading uptimerobot api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading uptimerobot provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached uptimerobot provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading uptimerobot cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/uptimerobot/testdata/uptimerobot_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting uptimerobot test data file path: %w", err)
	}
...
// FindHost searches for the host in the uptimerobot data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading uptimerobot host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning uptimerobot host match data")
