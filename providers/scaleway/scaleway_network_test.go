...
// upstream BGPView prefixes fixture (scaleway uses the bgpview provider), plus a
...
func newMockedClient(t *testing.T, status int, body []byte) *ProviderClient {
	...
	sess.Host = netip.MustParseAddr("51.15.0.1")
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/prefixes.json")
	require.NoError(t, err)
	...
	require.Equal(t, netip.MustParsePrefix("51.15.0.0/16"), parsed.Prefix)
}

func TestInitialiseNetworkFetchError(t *testing.T) {
	t.Parallel()

	// A non-200 response makes both the BGPView and RIPE stat fetches fail,
	// surfacing the error from loadProviderData.
