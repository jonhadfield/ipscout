...
func TestEnabled(t *testing.T) {
	pc := &ProviderClient{}
	enabled := true
	pc.Providers.Scaleway.Enabled = &enabled
	...
	pc.Providers.Scaleway.Enabled = nil
...
func TestLoadResultsFile(t *testing.T) {
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/scaleway_192_0_2_1_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix("192.0.2.0/24"), res.Prefix)
}
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["HostedInScaleway"])
}
...
func TestRateHostData(t *testing.T) {
	...
	// Simple rating config JSON with just the scaleway section
	ratingConfigJSON := `{
		"providers": {
			"scaleway": {
	...
	require.Equal(t, []string{"hosted in Scaleway"}, result.Reasons)
}
