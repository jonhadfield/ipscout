...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/zoom"
...
const (
	ProviderName = "zoom"
	// Zoom's published ranges had last changed two days before review, so the
	// list does move. A day keeps it current without polling a large static file.
	DocTTL = 24 * time.Hour
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating zoom client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.Zoom.Enabled != nil && *c.Providers.Zoom.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.Zoom.OutputPriority
}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in zoom data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.Zoom.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is Zoom"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling zoom data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching zoom data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("zoom data contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling zoom provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.Zoom.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.Zoom.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting zoom data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising zoom client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking zoom cache: %w", err)
	}

	if ok {
		c.Logger.Debug("zoom provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading zoom provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading zoom api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading zoom provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached zoom provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading zoom cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/zoom/testdata/zoom_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting zoom test data file path: %w", err)
	}
	...
	c.Logger.Info("zoom match returned from test data", "host", testDataHost)
...
// FindHost searches for the host in the zoom data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading zoom host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning zoom host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("Zoom | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("Zoom | Host: %s", testDataHost)
	}
