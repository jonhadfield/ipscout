...
// upstream perplexitybot fixture, plus a real temp cache, with UseTestData off.
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/perplexitybot.json")
	require.NoError(t, err)
...
func TestInitialiseEmptyDocumentNotCached(t *testing.T) {
	...
	body := []byte(`{"creationTime":"2025-02-07T16:56:00.000000","prefixes":[]}`)
	c := newMockedClient(t, http.StatusOK, body)
