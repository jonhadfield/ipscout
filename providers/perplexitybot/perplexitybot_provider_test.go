...
import (
	...
	fetcherPerplexitybot "github.com/jonhadfield/ip-fetcher/providers/perplexitybot"
...
// Values that match providers/perplexitybot/testdata/perplexitybot_18_97_9_100_report.json.
const (
	testHost         = "18.97.9.100"
	testPrefix       = "18.97.9.96/29"
	testDefaultScore = 7.5
	testCreationRFC  = "2025-02-07T16:56:00Z"
...
// seedCache writes a perplexitybot provider document into the cache so that the
...
func seedCache(t *testing.T, c *ProviderClient) {
	...
	doc := fetcherPerplexitybot.Doc{
		CreationTime: time.Date(2025, 2, 7, 16, 56, 0, 0, time.UTC),
		IPv4Prefixes: []fetcherPerplexitybot.IPv4Entry{
			{IPv4Prefix: netip.MustParsePrefix(testPrefix)},
		},
		IPv6Prefixes: []fetcherPerplexitybot.IPv6Entry{
...
func TestEnabled(t *testing.T) {
	...
	enabled := true
	pc.Providers.PerplexityBot.Enabled = &enabled
...
func TestPriority(t *testing.T) {
	...
	priority := int32(9)
	pc.Providers.PerplexityBot.OutputPriority = &priority
...
func TestExtractThreatIndicators(t *testing.T) {
	...
	require.Equal(t, "true", indicators.Indicators["PerplexityBot"])
}
...
func TestRateHostData(t *testing.T) {
	...
	require.Equal(t, []string{"source is PerplexityBot"}, result.Reasons)
}
...
func TestRateHostDataNoPrefix(t *testing.T) {
	...
	ratingConfigJSON := `{"providers":{"perplexitybot":{"defaultMatchScore":7.5}}}`
...
func TestLoadResultsFile(t *testing.T) {
	...
	res, err := providers.LoadResultsFile[HostSearchResult]("testdata/perplexitybot_18_97_9_100_report.json")
	require.NoError(t, err)
	require.Equal(t, netip.MustParsePrefix(testPrefix), res.Prefix)
	require.False(t, res.CreationTime.IsZero())
}
