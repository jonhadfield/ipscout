...
import (
	...
	ipfetcher "github.com/jonhadfield/ip-fetcher/providers/sentry"
...
const (
	ProviderName = "sentry"
	// Sentry publishes its uptime checker ranges as a static document with no
	// stated refresh interval. A day, matching the other monitoring providers.
	DocTTL = 24 * time.Hour
...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating sentry client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.Sentry.Enabled != nil && *c.Providers.Sentry.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.Sentry.OutputPriority
}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in sentry data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.Sentry.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"source is Sentry uptime monitoring"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling sentry data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching sentry data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("sentry data contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling sentry provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.Sentry.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.Sentry.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting sentry data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising sentry client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking sentry cache: %w", err)
	}

	if ok {
		c.Logger.Debug("sentry provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading sentry provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading sentry api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading sentry provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached sentry provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading sentry cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/sentry/testdata/sentry_192_0_2_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting sentry test data file path: %w", err)
	}
	...
	c.Logger.Info("sentry match returned from test data", "host", testDataHost)
...
// FindHost searches for the host in the sentry data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading sentry host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning sentry host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("Sentry | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("Sentry | Host: %s", testDataHost)
	}
