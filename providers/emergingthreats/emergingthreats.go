...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating emergingthreats client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.EmergingThreats.Enabled != nil && *c.Providers.EmergingThreats.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.EmergingThreats.OutputPriority
}
...
func (c *ProviderClient) ExtractThreatIndicators(findRes []byte) (*providers.ThreatIndicators, error) {
	...
	if doc.Prefix.IsValid() {
		indicators["CompromisedHost"] = "true"
	}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in emergingthreats data")
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling emergingthreats data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching emergingthreats data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("emergingthreats document contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling emergingthreats provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.EmergingThreats.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.EmergingThreats.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting emergingthreats data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising emergingthreats client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking emergingthreats cache: %w", err)
	}

	if ok {
		c.Logger.Debug("emergingthreats provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading emergingthreats provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading emergingthreats api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading emergingthreats provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached emergingthreats provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading emergingthreats cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/emergingthreats/testdata/emergingthreats_198_51_100_42_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting emergingthreats test data file path: %w", err)
	}
...
// FindHost searches for the host in the emergingthreats data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading emergingthreats host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning emergingthreats host match data")
