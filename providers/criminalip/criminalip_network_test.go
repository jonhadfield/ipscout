...
func newMockedClient(t *testing.T, status int, body []byte) *Client {
	...
	enabled := true
	sess.Providers.CriminalIP.Enabled = &enabled
	sess.Providers.CriminalIP.APIKey = "test-key"
...
func TestFindHostNetworkSuccess(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/criminalip_9_9_9_9_report.json")
	require.NoError(t, err)
...
func TestFindHostNetworkForbidden(t *testing.T) {
	t.Parallel()

	// A 200 response whose body reports a 403 status is treated as an
	// authorisation failure by loadAPIResponse.
	body := []byte(`{"status":403,"message":"invalid api key"}`)
