...
// upstream prefixes CSV fixture, plus a real temp cache, with UseTestData off.
func newMockedClient(t *testing.T, status int, body []byte) *ProviderClient {
	...
	sess.Host = netip.MustParseAddr("172.224.224.60")
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	body, err := os.ReadFile("testdata/prefixes.csv")
	require.NoError(t, err)
	...
	require.Equal(t, netip.MustParsePrefix("172.224.224.0/24"), parsed.Prefix)
	require.Equal(t, "GB", parsed.Alpha2Code)
	require.Equal(t, "London", parsed.City)
}
