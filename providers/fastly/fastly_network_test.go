...
const (
	testNetHost   = "151.101.0.1"
	testNetPrefix = "151.101.0.0/16"
	// testNetBody mirrors the upstream https://api.fastly.com/public-ip-list format.
	testNetBody = `{"addresses":["23.235.32.0/20","151.101.0.0/16"],"ipv6_addresses":["2a04:4e40::/32"]}`
)
...
// upstream public-ip-list body, plus a real temp cache, with UseTestData off.
func newMockedClient(t *testing.T, status int, body []byte) *ProviderClient {
	...
	sess.Host = netip.MustParseAddr(testNetHost)
...
func TestInitialiseAndFindHostOverNetwork(t *testing.T) {
	...
	c := newMockedClient(t, http.StatusOK, []byte(testNetBody))
	...
	require.Equal(t, netip.MustParsePrefix(testNetPrefix), parsed.Prefix)
}

func TestInitialiseNetworkFetchError(t *testing.T) {
	t.Parallel()

	// ip-fetcher v0.0.18 ignores the HTTP status for fastly; fetch errors
	// surface from body parsing, so an unparseable body makes Fetch fail.
...
func TestInitialiseNetworkEmptyDocument(t *testing.T) {
	...
	// A valid JSON body with no address lists (e.g. an upstream JSON error
	// object served with a status ip-fetcher ignores) parses to an empty Doc;
	// the empty-document guard must reject it rather than cache it.
	c := newMockedClient(t, http.StatusOK, []byte(`{"msg":"internal server error"}`))
