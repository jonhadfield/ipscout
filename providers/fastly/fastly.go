...
func NewProviderClient(c session.Session) (providers.ProviderClient, error) {
	c.Logger.Debug("creating fastly client")

	return &ProviderClient{Session: c}, nil
}

func (c *ProviderClient) Enabled() bool {
	if c.UseTestData || (c.Providers.Fastly.Enabled != nil && *c.Providers.Fastly.Enabled) {
		return true
	}
...
func (c *ProviderClient) Priority() *int32 {
	return c.Providers.Fastly.OutputPriority
}
...
func (c *ProviderClient) ExtractThreatIndicators(findRes []byte) (*providers.ThreatIndicators, error) {
	...
	if doc.Prefix.IsValid() {
		indicators["HostedInFastly"] = "true"
	}
...
func (c *ProviderClient) RateHostData(findRes []byte, ratingConfigJSON []byte) (providers.RateResult, error) {
	...
	if doc.Prefix.String() == "" {
		return rateResult, errors.New("no prefix found in fastly data")
	}

	if doc.Prefix.IsValid() {
		rateResult.Score = ratingConfig.ProviderRatingsConfigs.Fastly.DefaultMatchScore
		rateResult.Detected = true
		rateResult.Reasons = []string{"hosted in Fastly"}
	}
...
func unmarshalProviderData(data []byte) (*ipfetcher.Doc, error) {
	...
	if err := json.Unmarshal(data, &res); err != nil {
		return nil, fmt.Errorf("error unmarshalling fastly data: %w", err)
	}
...
func (c *ProviderClient) loadProviderData() error {
	...
	if err != nil {
		return fmt.Errorf("error fetching fastly data: %w", err)
	}
	...
	if len(doc.IPv4Prefixes) == 0 && len(doc.IPv6Prefixes) == 0 {
		return fmt.Errorf("fastly data contains no prefixes: %w", providers.ErrFailedToFetchData)
	}

	data, err := json.Marshal(doc)
	if err != nil {
		return fmt.Errorf("error marshalling fastly provider doc: %w", err)
	}

	docCacheTTL := DocTTL
	if c.Providers.Fastly.DocumentCacheTTL != 0 {
		docCacheTTL = time.Minute * time.Duration(c.Providers.Fastly.DocumentCacheTTL)
	}
	...
	if err != nil {
		return fmt.Errorf("error upserting fastly data: %w", err)
	}
...
func (c *ProviderClient) Initialise() error {
	...
	c.Logger.Debug("initialising fastly client")

	ok, err := cache.CheckExists(c.Logger, c.Cache, providers.CacheProviderPrefix+ProviderName)
	if err != nil {
		return fmt.Errorf("checking fastly cache: %w", err)
	}

	if ok {
		c.Logger.Debug("fastly provider data found in cache")

		return nil
	}

	c.Logger.Debug("loading fastly provider data from source")

	err = c.loadProviderData()
	if err != nil {
		return fmt.Errorf("loading fastly api response: %w", err)
	}
...
func (c *ProviderClient) loadProviderDataFromCache() (*ipfetcher.Doc, error) {
	c.Logger.Debug("loading fastly provider data from cache")
	...
	if item, err := cache.Read(c.Logger, c.Cache, cacheKey); err == nil {
		...
		if uErr != nil {
			...
			return nil, fmt.Errorf("error unmarshalling cached fastly provider doc: %w", uErr)
		}
	} else {
		return nil, fmt.Errorf("error reading fastly cache: %w", err)
	}
...
func loadTestData(c *ProviderClient) ([]byte, error) {
	resultsFile, err := helpers.PrefixProjectRoot("providers/fastly/testdata/fastly_151_101_0_1_report.json")
	if err != nil {
		return nil, fmt.Errorf("error getting fastly test data file path: %w", err)
	}
	...
	c.Logger.Info("fastly match returned from test data", "host", "151.101.0.1")
...
// FindHost searches for the host in the fastly data
func (c *ProviderClient) FindHost() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading fastly host data from cache: %w", err)
	}
	...
	for _, p := range doc.IPv4Prefixes {
		if p.Contains(c.Host) {
			...
			c.Logger.Debug("returning fastly host match data")
...
func (c *ProviderClient) CreateTable(data []byte) (*table.Writer, error) {
	...
	tw.SetTitle("Fastly | Host: %s", c.Host.String())

	if c.UseTestData {
		tw.SetTitle("Fastly | Host: %s", "151.101.0.1")
	}
